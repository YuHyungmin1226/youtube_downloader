...
- **YouTube 미디어 403 자동 복구**: 고화질 요청이 차단되면 통합 스트림 호환 프로필로 자동 재시도
- **YouTube 고화질 프로필**: 기본 `tv_embedded` 프로필에서 1080p 이상을 우선 시도하고, 차단되면 `android_vr`와 `android`로 자동 전환합니다.
- **YouTube PO Token 지원**: 일부 요청 프로필은 영상별 PO Token이 필요할 수 있으며, 설정에서 `android_vr.gvs+토큰` 형식으로 입력할 수 있습니다.
...
- **재생목록/채널 제한**: 재생목록 또는 채널 동영상 탭에서 다운로드할 영상 개수 제한
- **쿠키 파일**: 로그인 세션 유지를 위한 쿠키 파일 경로
- **YouTube 요청 프로필**: `TV Embedded (고화질 우선)`을 기본으로 사용하며, 문제가 있을 때 Android 프로필로 전환할 수 있습니다.
